...
import uk.ac.cam.jp775.oop.tick3.Pattern;
import uk.ac.cam.jp775.oop.tick3.PatternFormatException;
import uk.ac.cam.jp775.oop.tick3.PatternNotFound;
...
public class PatternStore {
	...
	public List<Pattern> getPatternsAuthorSorted() {
		// Collections.sort(mPatterns, Comparator.comparing(p1 -> p1.getAuthor())
		// .thenComparing(p1 -> p1.getName()));
	...
	public List<String> getNumPatternsRepresentableBy(int x) {
		return getPatternNames().stream()
				.filter(s -> getPatternByName(s).getHeight() * getPatternByName(s).getWidth() <= x)
				.map(s -> s + " " + getPatternByName(s).getHeight() + " " + getPatternByName(s).getWidth()).sorted()
